...
基于 Spring Cloud Alibaba 的外卖业务微服务项目，包含用户端、管理端、网关以及多个独立业务服务模块，支持后端、本地前端静态资源和微信小程序联调。
...
1. 解压后使用微信开发者工具导入项目。
2. `project.config.json` 中的 AppID 仅作为本地调试示例，公开仓库前请替换为你自己的 AppID，或者只保留压缩包不提交解压后的目录。
3. 将请求地址配置为你的网关地址（例如 `http://localhost:8080`）。
4. 使用你自己的小程序 AppID 与后端登录配置联调。
